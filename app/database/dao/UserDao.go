...
//
// @return `[]User`
func (u *userDao) QueryAllUsers() (users []po2.User) {
...
func (u *userDao) QueryUserByUid(uid int) (*po2.User, bool) {
	var user po2.User
	nf := database.DB.Where(col_user_uid+" = ?", uid).Find(&user).RecordNotFound()
	if nf {
		return nil, false
	} else {
		user.ToServer()
...
//
// @return `*user` `isUserExist`
func (u *userDao) QueryUserByUserName(username string) (*po2.User, bool) {
	var user po2.User
	nf := database.DB.Where(col_user_username+" = ?", username).Find(&user).RecordNotFound()
	if nf {
		return nil, false
	} else {
		user.ToServer()
...
//
// @return `[]user`
func (u *userDao) SearchByUserName(username string) (users []po2.User) {
	database.DB.Where(col_user_username+" like ?", "%"+username+"%").Find(&users).RecordNotFound()
	for _, v := range users {
		v.ToServer()
	}
	return users
...
func (u *userDao) UpdateUser(user po2.User) (*po2.User, error) {
	// 检查用户信息
	...
	// if user.Profile == config.AppConfig.MagicToken {
...
func (u *userDao) DeleteUser(uid int) (*po2.User, error) {
...
func (u *userDao) SubscribeUser(suberUid int, upUid int) error {
	...
	database.DB.Model(upUser).Association("Subscribers").Append(suberUser)
	return nil
}
...
func (u *userDao) UnSubscribeUser(suberUid int, upUid int) error {
	...
	database.DB.Model(upUser).Association("Subscribers").Delete(suberUser)
	return nil
}
...
func (u *userDao) QuerySubscriberUsers(uid int) ([]po2.User, error) {
	user, ok := u.QueryUserByUid(uid)
	if !ok {
		return nil, UserNotExistException
	}
	var users []po2.User
	database.DB.Model(user).Related(&users, "Subscribers")
	for _, v := range users {
		v.ToServer()
	}
	// SELECT `tbl_user`.*
...
//
// @error `UserNotExistException`
func (u *userDao) QuerySubscribingUsers(uid int) ([]po2.User, error) {
	user, ok := u.QueryUserByUid(uid)
	if !ok {
		return nil, UserNotExistException
	}
	var users []po2.User
	database.DB.Model(user).Related(&users, "Subscribings")
	for _, v := range users {
		v.ToServer()
	}
	// SELECT `tbl_user`.*
...
func (u *userDao) QuerySubCnt(uid int) (int, int, error) {
	...
	var subing []po2.User
	database.DB.Model(user).Related(&subing, "Subscribings")
	var suber []po2.User
	database.DB.Model(user).Related(&suber, "Subscribers")
...
//
// @error `UserNotExistException`
func (u *userDao) QueryUserExtraInfo(isAuth bool, user *po2.User) (*resp.UserExtraInfo, error) {
